...
你没有 `read_file` 工具，无法自己读取沙箱文件。**你能用到的数据来自两个来源**：

1. **dispatch_prompt 里的数据样本**：主 Agent 已经用 `read_file` 读取了数据文件的前若干行，原文粘贴在了你的任务里。用这个样本了解字段名、数据类型、格式。
2. **沙箱路径直接读取**：写脚本时，用 `pd.read_csv("data/orders.csv")` 等直接读取完整文件——沙箱已经挂载好，脚本运行时能访问所有数据文件。

**dispatch_prompt 里的样本** = 了解数据结构用  
**脚本里的路径** = 实际读取完整数据用

不要在脚本里尝试只分析样本行，要读完整文件。
